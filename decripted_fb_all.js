/*1291425966,169910391,JIT Construction: v320840,en_US*/
if (!window.FB)
{
    FB =
    {
        _apiKey : null, _session : null, _userStatus : 'unknown', _logging : true, _inCanvas : ((window.location.search.indexOf('fb_sig_in_iframe=1') > - 1) || (window.location.search.indexOf('session=') > - 1) || (window.location.search.indexOf('signed_request=') > - 1)),
        _domain :
        {
            api : 'https://api.facebook.com/', api_read : 'https://api-read.facebook.com/', cdn : (window.location.protocol == 'https:' ? 'https://s-static.ak.fbcdn.net/' : 'http://static.ak.fbcdn.net/'),
            graph : 'https://graph.facebook.com/', staticfb : 'http://static.ak.facebook.com/', staticfbSsl : 'https://s-static.ak.facebook.com/',
            www : window.location.protocol + '//www.facebook.com/', wwwSsl : 'https://www.facebook.com/'
        },
        _locale : null, _localeIsRtl : false,
...
FB.provide('Flash',
{
    _minVersions : [[9, 0, 159, 0], [10, 0, 22, 87]], _swfPath : 'swf/XdComm.swf', _callbacks : [],
    init : function ()
    {
        if (FB.Flash._init) {
            return;
...
{
    METHODS : ['get', 'post', 'delete', 'put'], _callbacks : {}, _readOnlyCalls :
    {
        fql_query : true, fql_multiquery : true, friends_get : true, notifications_get : true, stream_get : true,
        users_getinfo : true
    },
    graph : function ()
    {
    ...
    {
        if (FB._session && FB._session.access_token && !e.access_token) {
            e.access_token = FB._session.access_token;
        }
        e.sdk = 'joey';
        e.pretty = 0;
        var d = a;
        a = function (h)
        {
            if (FB.Auth && h && FB._session && FB._session.access_token == e.access_token && (h.error_code === '190' || (h.error && (h.error === 'invalid_token' || h.error.type === 'OAuthException')))) {
...
FB.provide('Dom',
{
    ...
    getBrowserType : function ()
    {
        ...
        {
            var d = window.navigator.userAgent.toLowerCase(), b = ['msie', 'firefox', 'safari', 'gecko'],
    ...
    getViewportInfo : function ()
    {
        ...
        return {
            scrollTop : a.scrollTop, scrollLeft : a.scrollLeft, width : self.innerWidth ? self.innerWidth : a.clientWidth,
...
FB.provide('Dialog',
{
    _loaderEl : null, _stack : [], _active : null,
    ...
    _makeActive : function (b)
    {
        ...
        var a = {
            width : parseInt(b.offsetWidth, 10), height : parseInt(b.offsetHeight, 10)
        },
    ...
    create : function (e)
    {
        e = e || {};
        if (e.loader) {
            FB.Dialog._showLoader(e.onClose, e.loaderWidth);
        }
        ...
        {
            a += ' fb_dialog_legacy';
            FB.Array.forEach(['vert_left', 'vert_right', 'horiz_top', 'horiz_bottom', 'top_left', 'top_right',
            'bottom_left', 'bottom_right'], function (g)
            {
...
FB.provide('XD',
{
    _origin : null, _transport : null, _callbacks : {}, _forever : {},
    init : function (a)
    {
        if (FB.XD._origin) {
            return;
    ...
    handler : function (a, e, b)
    {
        ...
        var f = FB._domain.cdn + 'connect/xd_proxy.php#', c = FB.guid();
    ...
    PostMessage :
    {
        init : function ()
        {
            ...
            window.addEventListener ? window.addEventListener('message', a, false) : window.attachEvent('onmessage',
            a);
        },
...
FB.provide('Arbiter',
{
    inform : function (b, d, e, g)
    {
        ...
        var h = (FB._domain[g ? 'staticfbSsl' : 'staticfb'] + 'connect/canvas_proxy.php#' + FB.QS.encode({
            method : b, params : FB.JSON.stringify(d || {}), relation : e
        }));
        var f = FB.Content.appendHidden('');
        FB.Content.insertIframe(
        {
            url : h, root : f, width : 1, height : 1,
            onload : function ()
            {
                setTimeout(function ()
                {
...
FB.provide('UIServer',
{
    Methods : {}, _active : {}, _defaultCb : {}, _resultToken : '"xxRESULTTOKENxx"',
    ...
    {
        var g = h.method.toLowerCase(), f = FB.UIServer.Methods[g], e = FB.guid(), d = (g !== 'auth.status');
        if (!f) {
        ...
        FB.copy(h,
        {
            api_key : FB._apiKey, locale : FB._locale, sdk : 'joey', access_token : d && FB._session && FB._session.access_token || undefined
        });
        h.display = FB.UIServer.getDisplayMode(f, h);
        if (!f.url) {
            f.url = 'connect/uiserver.php';
        }
        var a = {
            cb : b, id : e, size : f.size || {}, url : FB._domain[d ? 'wwwSsl' : 'www'] + f.url, params : h
        };
        ...
        if (i === 'parent') {
            a.params.channel_url = FB.UIServer._xdChannelHandler(e, 'parent.parent');
        }
    ...
    popup : function (b)
    {
        var a = typeof window.screenX != 'undefined' ? window.screenX : window.screenLeft, i = typeof window.screenY != 'undefined' ? window.screenY : window.screenTop,
        ...
        k = b.size.width, d = b.size.height, h = (a < 0) ? window.screen.width + a : a, e = parseInt(h + ((g - k) / 2),
        10), j = parseInt(i + ((f - d) / 2.5), 10), c = ('width=' + k + ',height=' + d + ',left=' + e + ',top=' + j + ',scrollbars=1');
        if (b.params.method == 'permissions.request') {
        ...
        if (b.post)
        {
            ...
            FB.UIServer._active[b.id] = window.open(b.url, b.id, c);
        }
        if (b.id in FB.UIServer._defaultCb) {
    ...
    _insertIframe : function (b)
    {
        ...
        var a = function (c)
        {
            if (b.id in FB.UIServer._active) {
                FB.UIServer._active[b.id] = c;
            }
        };
        if (b.post)
        {
            FB.Content.insertIframe(
            {
                url : 'about:blank', root : b.root, className : b.className, width : b.size.width, height : b.size.height,
                onload : function (c)
                {
                    a(c);
                    FB.Content.postTarget({
        ...
        else
        {
            FB.Content.insertIframe(
            {
                url : b.url, root : b.root, className : b.className, width : b.size.width, height : b.size.height,
                onload : a
            });
        }
    },
...
FB.provide('Auth',
{
    ...
    {
        var b = !FB._session && e, c = FB._session && !e, a = FB._session && e && FB._session.uid != e.uid,
        f = b || c || (FB._session && e && FB._session.access_token != e.access_token), h = g != FB._userStatus;
        var d = {
            session : e, status : g
        };
        FB._session = e;
        ...
        if (FB.Auth._loadState && e && e.expires)
        {
...
FB.provide('UIServer.Methods',
{
    ...
    'auth.status' :
    {
        ...
        transform : function (a)
        {
            ...
            FB.copy(a.params,
            {
                no_session : d(b, c, 'parent', false, 'notConnected'), no_user : d(b, c, 'parent', false,
                'unknown'), ok_session : d(b, c, 'parent', false, 'connected'), session_version : 3, extern : FB._inCanvas ? 0 : 2
            });
            return a;
        }
    }
...
FB.provide('Canvas',
{
    ...
    setSize : function (b)
    {
        ...
        if (FB.Canvas._lastSize[b.frame])
        {
            ...
            if (FB.Canvas._lastSize[b.frame].width == b.width && (b.height <= a && (a - b.height <= 16))) {
                return false;
            }
        }
    ...
    setAutoResize : function (b, a)
    {
        ...
        if (b === undefined || b)
        {
            if (FB.Canvas._timer === null) {
                FB.Canvas._timer = window.setInterval(FB.Canvas.setSize, a || 100);
            }
            FB.Canvas.setSize();
        }
    ...
    {
        var a = document.body, c = document.documentElement, d = 0, b = Math.max(Math.max(a.offsetHeight,
        a.scrollHeight) + a.offsetTop, Math.max(c.offsetHeight, c.scrollHeight) + c.offsetTop);
...
FB.provide('UIServer.Methods',
{
    ...
    'fbml.dialog' :
    {
        ...
        url : 'render_fbml.php', loggedOutIframe : true,
        transform : function (a)
        {
            return a;
        }
...
FB.provide('XFBML',
{
    ...
    parse : function (c, a)
    {
        ...
        {
            if (b > 0) {
                FB.log(b + ' XFBML tags failed to render in ' + FB.XFBML._renderTimeout + 'ms.');
            }
        },
    ...
    toFql : function ()
    {
        ...
        switch (this.where.type)
        {
            ...
            case 'in':
                ...
                {
                    a += this.where.key + ' in (' + FB.Array.map(this.where.value, this._encode).join(',') + ')';
                }
                break;
        }
        return a;
...
{
    _showLoader : true, _refreshOnAuthChange : false, _allowReProcess : false, _fetchPreCachedLoader : false,
    _visibleAfter : 'load',
    getUrlBits : function ()
    {
    ...
    process : function (a)
    {
        ...
        FB.Content.insertIframe(
        {
            url : d, root : this.dom.appendChild(document.createElement('span')), name : this.getIframeName(),
            title : this.getIframeTitle(), className : FB._localeIsRtl ? 'fb_rtl' : 'fb_ltr', height : c.height,
            width : c.width, onload : FB.bind(this.fire, this, 'iframe.onload')
        });
    },
    ...
    _getQS : function ()
    {
        return FB.copy(
        {
            api_key : FB._apiKey, locale : FB._locale, sdk : 'joey', session_key : FB._session && FB._session.session_key,
...
{
    _visibleAfter : 'load', _refreshOnAuthChange : true,
    setupAndValidate : function ()
    {
        this._attr =
        {
            border_color : this.getAttribute('border-color'), colorscheme : this.getAttribute('color-scheme'),
            filter : this.getAttribute('filter'), font : this.getAttribute('font'), header : this._getBoolAttribute('header'),
            height : this._getPxAttribute('height', 300), recommendations : this._getBoolAttribute('recommendations'),
            site : this.getAttribute('site', location.hostname), width : this._getPxAttribute('width',
            300)
        };
        return true;
    },
...
{
    _visibleAfter : 'resize', _refreshOnAuthChange : true,
    setupAndValidate : function ()
    {
        var a =
        {
            channel_url : this.getChannelUrl(), css : this.getAttribute('css'), notify : this.getAttribute('notify'),
            numposts : this.getAttribute('num-posts', 10), quiet : this.getAttribute('quiet'), reverse : this.getAttribute('reverse'),
            simple : this.getAttribute('simple'), title : this.getAttribute('title', document.title),
            url : this.getAttribute('url', document.URL), width : this._getPxAttribute('width', 550),
            xid : this.getAttribute('xid'), href : this.getAttribute('href')
        };
        if (!a.xid)
        {
        ...
        var e = 40, f = {}, j = {}, a = null, h = c.style, i = setInterval(FB.bind(function ()
        {
            ...
            FB.Array.forEach(g, FB.bind(function (o, m)
            {
                ...
                FB.Array.forEach(f[m], function (q, p)
                {
                    ...
                    {
                        l += (q.numPart + Math.ceil((j[m][p].numPart - q.numPart) * Math.sin(Math.PI  / 2 * k))) + j[m][p].textPart + ' ';
                    }
                });
...
{
    _initialHeight : null, _initTopMargin : 0, _picFieldName : 'pic_square', _page : null, _displayed : false,
    _notDisplayed : false, _container : null, _animationSpeed : 0,
    process : function ()
    {
    ...
    _showBar : function ()
    {
        var a = FB.Data._selectByIndex(['first_name', 'profile_url', this._picFieldName], 'user', 'uid',
        this._uid);
    ...
    _renderConnectBar : function (d)
    {
        ...
        this._container = c;
        this._initialHeight = Math.round(parseFloat(FB.Dom.getStyle(c, 'height')) + parseFloat(FB.Dom.getStyle(c,
        'borderBottomWidth')));
        b.innerHTML = FB.String.format('<div class="fb_buttons">' + '<a href="#" class="fb_bar_close">' + '<img src="{1}" alt="{2}" title="{2}"/>' + '</a>' + '</div>' + '<a href="{7}" class="fb_profile" target="_blank">' + '<img src="{3}" alt="{4}" title="{4}"/>' + '</a>' + '{5}' + ' <span>' + '<a href="{8}" class="fb_learn_more" target="_blank">{6}</a> &ndash; ' + '<a href="#" class="fb_no_thanks">{0}</a>' + '</span>',
        FB.Intl._tx("No Thanks"), FB._domain.cdn + FB.XFBML.ConnectBar.imgs.buttonUrl, FB.Intl._tx("Close"),
        d[this._picFieldName] || FB._domain.cdn + FB.XFBML.ConnectBar.imgs.missingProfileUrl, FB.String.escapeHTML(d.first_name),
        FB.Intl._tx("Hi {firstName}. <strong>{siteName}<\/strong> is using Facebook to personalize your experience.",

        {
            firstName : FB.String.escapeHTML(d.first_name), siteName : FB.String.escapeHTML(d.site_name)
        }), FB.Intl._tx("Learn More"), d.profile_url, FB._domain.www + 'sitetour/connect.php');
        var a = this;
        ...
        {
            f = Math.round((parseFloat(FB.Dom.getStyle(this._page.parentNode, 'height')) - parseFloat(FB.Dom.getStyle(this._page,
            'height'))) / 2);
        }
        else {
            f = parseInt(FB.Dom.getStyle(this._page, 'marginTop'), 10);
        }
        f = isNaN(f) ? 0 : f;
...
FB.subclass('XFBML.Facepile', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            href : this.getAttribute('href'), channel : this.getChannelUrl(), max_rows : this.getAttribute('max-rows'),
            width : this._getPxAttribute('width', 200), ref : this.getAttribute('ref')
        };
        for (var a in this._extraParams) {
            this._attr[a] = this._extraParams[a];
        }
        return true;
    },
...
FB.subclass('XFBML.Fan', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            api_key : FB._apiKey, connections : this.getAttribute('connections', '10'), css : this.getAttribute('css'),
            height : this._getPxAttribute('height'), id : this.getAttribute('profile-id'), logobar : this._getBoolAttribute('logo-bar'),
            name : this.getAttribute('name'), stream : this._getBoolAttribute('stream', true), width : this._getPxAttribute('width',
            300)
        };
...
FB.subclass('XFBML.EdgeCommentWidget', 'XFBML.IframeWidget', function (a)
{
    ...
    if (a.relativeWidthOffset)
    {
        if (FB._localeIsRtl) {
            this.dom.style.right = a.relativeWidthOffset;
        }
        else {
            this.dom.style.left = a.relativeWidthOffset;
        }
...
{
    _visibleAfter : 'load', _showLoader : false,
    getSize : function ()
    {
        return {
...
{
    _visibleAfter : 'immediate', _showLoader : false,
    setupAndValidate : function ()
    {
        FB.Dom.addCss(this.dom, 'fb_edge_widget_with_comment');
        this._attr =
        {
            channel_url : this.getChannelUrl(), debug : this._getBoolAttribute('debug'), href : this.getAttribute('href',
            window.location.href), is_permalink : this._getBoolAttribute('is-permalink'), node_type : this.getAttribute('node-type',
            'link'), width : this._getWidgetWidth(), font : this.getAttribute('font'), layout : this._getLayout(),
            colorscheme : this.getAttribute('color-scheme'), action : this.getAttribute('action'), ref : this.getAttribute('ref'),
            show_faces : this._shouldShowFaces(), no_resize : this._getBoolAttribute('no_resize')
        };
        return true;
    },
    oneTimeSetup : function ()
    {
        ...
        this.subscribe('xd.presentEdgeCommentDialog', FB.bind(this._handleEdgeCommentDialogPresentation,
        this));
        this.subscribe('xd.dismissEdgeCommentDialog', FB.bind(this._handleEdgeCommentDialogDismissal,
        this));
    ...
    _createEdgeCommentWidget : function (b, a)
    {
        var c =
        {
            commentNode : a, externalUrl : b.externalURL, width : 330, height : 200, masterFrameName : b.masterFrameName,
            layout : this._getLayout(), relativeHeightOffset : this._getHeightOffset(), relativeWidthOffset : this._getWidthOffset(b)
        };
    ...
    _getHeightOffset : function ()
    {
        var a = this._getLayout();
        var b = {
            standard : '20px', button_count : '17px', box_count : '-5px'
        };
        return b[a];
    },
    _getWidthOffset : function (c)
    {
        if (c.preComputedWidthOffset) {
            return parseInt(c.preComputedWidthOffset, 10) + 'px';
        }
        var a = this._getLayout();
        var b = {
            standard : '17px', box_count : '0px', button_count : '0px'
        };
        return b[a];
    },
...
FB.subclass('XFBML.LikeBox', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            channel : this.getChannelUrl(), api_key : FB._apiKey, connections : this.getAttribute('connections',
            '10'), css : this.getAttribute('css'), height : this.getAttribute('height'), id : this.getAttribute('profile-id'),
            header : this._getBoolAttribute('header', true), name : this.getAttribute('name'), stream : this._getBoolAttribute('stream',
            true), width : this._getPxAttribute('width', 300), show_faces : this._getBoolAttribute('show-faces',
            false), href : this.getAttribute('href'), colorscheme : this.getAttribute('colorscheme', 'light')
        };
...
FB.subclass('XFBML.LiveStream', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            height : this._getPxAttribute('height', 500), hideFriendsTab : this.getAttribute('hide-friends-tab'),
            redesigned : this._getBoolAttribute('redesigned-stream'), width : this._getPxAttribute('width',
            400), xid : this.getAttribute('xid', 'default'), always_post_to_friends : this._getBoolAttribute('always-post-to-friends',
            false)
        };
        return true;
    },
...
FB.subclass('XFBML.LoginButton', 'XFBML.ButtonElement', null,
{
    setupAndValidate : function ()
    {
        ...
        this._attr =
        {
            autologoutlink : this._getBoolAttribute('auto-logout-link'), length : this._getAttributeFromList('length',
            'short', ['long', 'short']), onlogin : this.getAttribute('on-login'), perms : this.getAttribute('perms'),
            registration_url : this.getAttribute('registration-url'), status : 'unknown'
        };
...
FB.subclass('XFBML.Name', 'XFBML.Element', null,
{
    process : function ()
    {
        FB.copy(this,
        {
            _uid : this.getAttribute('uid'), _firstnameonly : this._getBoolAttribute('first-name-only'),
            _lastnameonly : this._getBoolAttribute('last-name-only'), _possessive : this._getBoolAttribute('possessive'),
            _reflexive : this._getBoolAttribute('reflexive'), _objective : this._getBoolAttribute('objective'),
            _linked : this._getBoolAttribute('linked', true), _subjectId : this.getAttribute('subject-id')
        });
        if (!this._uid) {
        ...
        FB.Event.monitor('auth.statusChange', this.bind(function ()
        {
            ...
            if (FB.Helper.isUser(this._uid)) {
                ...
                a = FB.Data._selectByIndex(['name', 'id'], 'profile', 'id', this._uid);
            }
    ...
    _renderOther : function (c)
    {
        ...
        else
        {
            ...
            if (this._firstnameonly) {
                ...
                b = FB.String.escapeHTML(c.last_name);
            }
            if (!b) {
                b = FB.String.escapeHTML(c.name);
            }
        ...
        if (!b) {
            b = FB.String.escapeHTML(this.getAttribute('if-cant-see', 'Facebook User'));
        }
        if (b)
        {
            if (this._getBoolAttribute('capitalize', false)) {
...
FB.subclass('XFBML.ProfilePic', 'XFBML.Element', null,
{
    process : function ()
    {
        var d = this.getAttribute('size', 'thumb'), b = FB.XFBML.ProfilePic._sizeToPicFieldMap[d], g = this._getPxAttribute('width'),
        a = this._getPxAttribute('height'), e = this.dom.style, f = this.getAttribute('uid');
        ...
        var c = this.bind(function (j)
        {
            ...
            if (!i) {
                i = FB._domain.cdn + FB.XFBML.ProfilePic._defPicMap[b];
            }
            var k = ((g ? 'width:' + g + ';' : '') + (a ? 'height:' + g + ';' : '')), h = FB.String.format('<img src="{0}" alt="{1}" title="{1}" style="{2}" class="{3}" />',
            i, l ? FB.String.escapeHTML(l.name) : '', k, this.dom.className);
            if (this._getBoolAttribute('linked', true)) {
                h = FB.Helper.getProfileLink(l, h, this.getAttribute('href', null));
            }
        ...
        FB.Event.monitor('auth.statusChange', this.bind(function ()
        {
            ...
            if (this.getAttribute('uid', null) == 'loggedinuser') {
                f = FB.Helper.getLoggedInUser();
            }
            if (FB._userStatus && f)
            {
                FB.Data._selectByIndex(['name', b], FB.Helper.isUser(f) ? 'user' : 'profile', FB.Helper.isUser(f) ? 'uid' : 'id',
                f).wait(c);
            }
            else {
                c();
...
FB.provide('XFBML.ProfilePic',
{
    _defPicMap :
    {
        pic : 'pics/s_silhouette.jpg', pic_big : 'pics/d_silhouette.gif', pic_big_with_logo : 'pics/d_silhouette_logo.gif',
        pic_small : 'pics/t_silhouette.jpg', pic_small_with_logo : 'pics/t_silhouette_logo.gif', pic_square : 'pics/q_silhouette.gif',
        pic_square_with_logo : 'pics/q_silhouette_logo.gif', pic_with_logo : 'pics/s_silhouette_logo.gif'
    },
    _sizeToPicFieldMap :
    {
        n : 'pic_big', normal : 'pic_big', q : 'pic_square', s : 'pic', small : 'pic', square : 'pic_square',
...
FB.subclass('XFBML.Recommendations', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            border_color : this.getAttribute('border-color'), colorscheme : this.getAttribute('color-scheme'),
            filter : this.getAttribute('filter'), font : this.getAttribute('font'), header : this._getBoolAttribute('header'),
            height : this._getPxAttribute('height', 300), site : this.getAttribute('site', location.hostname),
...
FB.subclass('XFBML.Registration', 'XFBML.IframeWidget', null,
{
    ...
    setupAndValidate : function ()
    {
        this._attr =
        {
            channel_url : this.getChannelUrl(), client_id : FB._apiKey, height : this._getPxAttribute('height',
            500), width : this._getPxAttribute('width', 600), redirect_uri : this.getAttribute('redirect-uri',
            window.location.href), fb_only : this._getBoolAttribute('fb-only', false), fields : this.getAttribute('fields')
        };
        return true;
    },
    getSize : function ()
...
FB.subclass('XFBML.Send', 'XFBML.EdgeWidget', null,
{
    setupAndValidate : function ()
    {
        ...
        this._attr =
        {
            channel : this.getChannelUrl(), api_key : FB._apiKey, colorscheme : this.getAttribute('colorscheme',
            'light'), href : this.getAttribute('href', window.location.href)
        };
        return true;
    },
    getUrlBits : function ()
    ...
    _createEdgeCommentWidget : function (b, a)
    {
        var c =
        {
            colorscheme : this._attr.colorscheme, commentNode : a, controllerID : b.controllerID, nodeImageURL : b.nodeImageURL,
            nodeTitle : b.nodeTitle, nodeURL : b.nodeURL, nodeSummary : b.nodeSummary, width : 400, height : 300,
            relativeHeightOffset : this._getHeightOffset(), relativeWidthOffset : this._getWidthOffset()
        };
...
FB.subclass('XFBML.ShareButton', 'XFBML.Element', null,
{
    ...
    _renderButton : function (f)
    {
        ...
        var b = '', c = '', d = '', a = '', e = FB.Intl._tx("Share"), g = '';
        switch (this._type)
        {
            case 'icon':
            case 'icon_link':
        ...
        this.dom.innerHTML = FB.String.format('<span class="{0}">{4}<a href="{1}" class="{2}" ' + 'onclick=\'FB.ui({6});return false;\'' + 'target="_blank">{3}</a>{5}</span>',
        g, this._href, a, b, d, c, FB.JSON.stringify({
...
FB.provide("",
{
    "_domain" :
    {
        "api" : "https:\/\/api.facebook.com\/", "api_read" : "https:\/\/api-read.facebook.com\/", "cdn" : "http:\/\/static.ak.fbcdn.net\/",
        "graph" : "https:\/\/graph.facebook.com\/", "staticfb" : "http:\/\/static.ak.facebook.com\/",
        "staticfbSsl" : "https:\/\/s-static.ak.facebook.com\/", "www" : "http:\/\/www.facebook.com\/",
        "wwwSsl" : "https:\/\/www.facebook.com\/"
    },
    "_locale" : "en_US", "_localeIsRtl" : false
}, true);
FB.provide("Flash", {
    "_minVersions" : [[10, 0, 22, 87]], "_swfPath" : "rsrc.php\/v1\/zx\/r\/DmvbpGB-fMy.swf"
}, true);
FB.provide("XFBML.ConnectBar",
{
    "imgs" :
    {
        "buttonUrl" : "rsrc.php\/zY\/r\/h_Y6u1wrZPW.png", "missingProfileUrl" : "rsrc.php\/zo\/r\/UlIqmHJn-SK.gif"
    }
}, true);
FB.provide("XFBML.ProfilePic",
{
    "_defPicMap" :
    {
        "pic" : "rsrc.php\/zh\/r\/C5yt7Cqf3zU.jpg", "pic_big" : "rsrc.php\/zL\/r\/HsTZSDw4avx.gif", "pic_big_with_logo" : "rsrc.php\/z5\/r\/SRDCaeCL7hM.gif",
        "pic_small" : "rsrc.php\/zi\/r\/odA9sNLrE86.jpg", "pic_small_with_logo" : "rsrc.php\/zD\/r\/k1xiRXKnlGd.gif",
        "pic_square" : "rsrc.php\/zo\/r\/UlIqmHJn-SK.gif", "pic_square_with_logo" : "rsrc.php\/zX\/r\/9dYJBPDHXwZ.gif",
...
if (FB.Dom && FB.Dom.addCssRules)
{
    ...
    ["fb.css.base", "fb.css.dialog", "fb.css.iframewidget", "fb.css.button", "fb.css.connectbarwidget",
    "fb.css.edgecommentwidget", "fb.css.sendbuttonformwidget", "fb.css.sharebutton"]);
}
